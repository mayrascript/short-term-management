...
const Finance: React.FC = () => {
  ...
  const availableCategoryNames = categories.length > 0
    ? categories.map((item) => item.name)
    : DEFAULT_CATEGORIES;

  const formatMoney = (value: number, unit: 'COP' | 'USD') => new Intl.NumberFormat(
    undefined,
    { style: 'currency', currency: unit, maximumFractionDigits: 2 }
  ).format(value);

  const fetchExpenses = async (useFilter = filterActive, useTrashView = showTrash) => {
    try {
      const params: Record<string, string> = {};
      if (useFilter && startDate) params.startDate = startDate;
      if (useFilter && endDate) params.endDate = endDate;
      if (useTrashView) params.includeDeleted = 'true';
  ...
  const fetchCategories = async () => {
    try {
      ...
      const fetchedCategories: ExpenseCategory[] = Array.isArray(res.data) ? res.data : [];
      setCategories(fetchedCategories);
      if (fetchedCategories.length > 0) {
        const fetchedNames = fetchedCategories.map((item) => item.name);
        if (!fetchedNames.includes(category)) {
          setCategory(fetchedNames[0]);
        }
      }
    } catch (error) {
  ...
  const fetchExchangeRate = async () => {
    try {
      ...
      if (!payload?.rates?.COP) {
        throw new Error('Missing COP rate');
      }
  ...
  const handleCreateCategory = async () => {
    ...
    const existing = availableCategoryNames.find(
    ...
    try {
      const res = await api.post('/expense-categories', { name: trimmed });
  ...
  const handleRestore = async (expenseId: string) => {
    try {
      await api.patch(`/expenses/${expenseId}/restore`);
      if (editingExpenseId === expenseId) {
        resetEditForm();
      }
      fetchExpenses();
        ...
        <div className="card">
          ...
          <form onSubmit={handleCreate}>
            ...
            <div className="form-group">
              ...
              <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Optional details..." rows={3} />
            </div>
            <div className="form-group">
              <label className="form-label">Tags (comma separated)</label>
              <input type="text" value={tagsInput} onChange={e => setTagsInput(e.target.value)} placeholder="urgent, guest-related, monthly" />
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              ...
              <select value={currency} onChange={e => setCurrency(e.target.value as 'COP' | 'USD')}>
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <input type="text" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} placeholder="e.g. Utilities" />
                <button type="button" className="btn" onClick={handleCreateCategory}>Add</button>
              </div>
              {categoryMessage && <p style={{ marginTop: '6px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>{categoryMessage}</p>}
            </div>
            <div className="form-group">
          ...
          {editingExpenseId && (
            <div style={{ marginTop: '20px', borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
              ...
              <form onSubmit={handleUpdateExpense}>
                ...
                <div className="form-group">
                  <label className="form-label">Currency</label>
                  <select value={editCurrency} onChange={e => setEditCurrency(e.target.value as 'COP' | 'USD')}>
                    <option value="COP">COP</option>
                    <option value="USD">USD</option>
                  </select>
                </div>
                ...
                <div className="form-group">
                  <label className="form-label">Receipt URL</label>
                  <input type="url" value={editReceiptUrl} onChange={e => setEditReceiptUrl(e.target.value)} placeholder="https://..." />
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button type="submit" className="btn btn-primary" style={{ flex: 1 }}>Save Changes</button>
                  <button type="button" className="btn" onClick={resetEditForm}>Cancel</button>
                </div>
                {editMessage && (
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
              ...
              <select value={targetCurrency} onChange={e => setTargetCurrency(e.target.value as 'COP' | 'USD')}>
          ...
          <p style={{ marginTop: '-6px', marginBottom: '12px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
            Rates by <a href="https://www.exchangerate-api.com" target="_blank" rel="noreferrer">ExchangeRate-API</a>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap' }}>
            ...
            <button type="button" className="btn btn-primary" style={{ padding: '4px 12px', fontSize: '0.85rem' }} onClick={() => { setFilterActive(true); fetchExpenses(true); }}>
              Filter
            </button>
            {filterActive && (
              <button type="button" className="btn" style={{ padding: '4px 12px', fontSize: '0.85rem' }} onClick={() => { setFilterActive(false); fetchExpenses(false); }}>
                Clear Filter
              </button>
            )}
          </div>
          ...
          {loading ? <p>Loading...</p> : (
            <div className="table-container">
              <table>
                ...
                <tbody>
                  {expenses.length === 0 ? (
                    <tr><td colSpan={8} style={{textAlign: 'center'}}>{showTrash ? 'Trash is empty.' : 'No expenses logged.'}</td></tr>
                  ) : (
                    expenses.map(exp => (
                      <tr key={exp._id}>
                        ...
                        <td>{exp.tags && exp.tags.length > 0 ? exp.tags.join(', ') : '-'}</td>
                        <td>{exp.comment?.trim() ? exp.comment : '-'}</td>
                        <td>
                          {exp.receiptUrl ? (
                        ...
                        <td style={{ color: 'var(--danger)', fontWeight: 500 }}>
                          {formatMoney(exp.amount, exp.currency ?? 'COP')}
                        </td>
                        <td>
                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                            {showTrash ? (
                              <button type="button" className="btn btn-primary" style={{ padding: '4px 8px', fontSize: '0.8rem' }} onClick={() => handleRestore(exp._id)}>
                                Restore
                              </button>
                            ) : (
                              <>
                                <button type="button" className="btn" style={{ padding: '4px 8px', fontSize: '0.8rem' }} onClick={() => openEditForm(exp)}>
                                  Edit
                                </button>
                                <button type="button" className="btn" style={{ padding: '4px 8px', fontSize: '0.8rem' }} onClick={() => handleSoftDelete(exp._id)}>
                                  Delete
                                </button>
                              </>
                            )}
